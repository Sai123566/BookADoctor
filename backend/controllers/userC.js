...
const appointmentController = async (req, res) => {
  try {
    const { userEmail, doctorEmail, userInfo, doctorInfo, date } = req.body;

    // Step 1: Find user and doctor by email
    const user = await userSchema.findOne({ email: userEmail });
    const doctor = await userSchema.findOne({ email: doctorEmail });

    if (!user || !doctor) {
      return res.status(404).send({ success: false, message: "User or Doctor not found" });
    }

    // Step 2: Save appointment using fetched IDs
    ...
    // Step 3: Add notification to the doctor
